...
<head>
    ...
    @yield('meta')
    <title>Robinet - @yield('title')</title>
    ...
    <style>
        ...
        body.font-scheherazade,
        body.font-scheherazade *:not(.fa):not(.fas):not(.far):not(.fab):not(.fal):not(.fad):not([class*="fa-"]) {
            font-family: 'Scheherazade New', serif !important;
        }
    </style>
</head>
...
<body class="flex flex-col min-h-screen justify-between bg-white font-scheherazade">
    ...
    <main class="overflow-x-hidden h-full mt-25">
        @yield('content')
    </main>
