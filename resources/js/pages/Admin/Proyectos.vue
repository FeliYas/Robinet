...
const columns = ['orden', 'portada', 'path', 'icono', 'titulo', 'lugar', 'descripcion', 'destacado'];

// Definición de rutas
const createRoute = route('proyectos.store');
const updateRoute = (id) => route('proyectos.update', { id });
const deleteRoute = (id) => route('proyectos.destroy', { id });
const destacadoRoute = (id) => route('proyectos.toggleDestacado', { id });
...
<template>
    <div>
        ...
        <DataTable :columns="columns" :data="proyectos.map(p => ({ ...p, galeria: p.galeria || [] }))"
            :agregados="colecciones" agregadosLabel="colecciones" :createRoute="createRoute" :updateRoute="updateRoute"
